...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    const prompt = [
      '你是企业知识库结构化整理助手。只输出加粗文档标题和操作步骤，其余内容一律不输出。',
      '',
      '输出格式严格如下，标题必须加粗，每条步骤由操作内容与对应备注合并为一行：',
      '**文档标题**',
      '',
      '### 操作步骤',
      '1. **接收工单**：客服、客响发送工单及客服邮件。备注：无工单后续客服补工单。',
      '2. **实施上架**：打开机柜门并固定设备。备注：避免设备滑落砸到脚。',
      '',
      '规则：',
      '1. 文档标题使用 **加粗**；',
      '2. 每条步骤的编号和步骤名称加粗，例如 **1. 接收工单**；',
      '3. 每条步骤必须把操作内容和对应备注合并到同一行；',
      '4. 备注前使用“备注：”标识；',
      '5. 删除页眉页脚、水印、页码、模板说明、联系方式、元信息、要求、完成标准和无关内容；',
      '6. 不要输出独立的“备注”分区，不要输出无步骤标题的要点。',
      `附件名称：${fileName}`,
      '',
      '附件内容：',
      content,
